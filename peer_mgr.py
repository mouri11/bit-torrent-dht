import time, select,loggin,errno,socket,random
from threading import Thread
from pubsub import pub
import msg,peers

class PeersMgr(Thread):
    def __init__(self, torrent, pieces_mgr):
        Thread.__init__(self)
        self.peers = []
        self.torrent = torrent
        self.pieces_manager = pieces_mgr
        self.pieces_by_peer = [[0, []] for _ in range(pieces_mgr.no_of_pieces)]
        self.is_active = True

        # Events
        pub.subscribe(self.peer_requests_piece, 'PeersMgr.PeerRqstsPiece')
        pub.subscribe(self.peers_bitfield, 'PeersMgr.updatePeersBitfield')

    def peer_rqsts_piece(self, rqst=None, peer=None):
        if not rqst or not peer:
            logging.error("empty request/peer message")

        piece_index, block_offset, block_length = rqst.piece_index, rqst.block_offset, rqst.block_length

        block = self.pieces_manager.get_block(piece_index, block_offset, block_length)
        if block:
            piece = message.DataPieces(piece_index, block_offset, block_length, block).to_bytes()
            peers.send_to_peer(piece)
            logging.info("Sent piece index {} to peer : {}".format(request.piece_index, peers.ip))
    ...
    def _process_new_message(self, new_message: message.Message, peer: peers.Peer):
